 ...
 * 브라우저가 했던 서버와의 통신 부분이 자바스크립트 조작을 통해 가능하다.
...
//callback, rul만 던져도 됨. 그럼 뒤에 method, params는 공백
 function sendRequest(callback, url, method, params) {
 	...
 	// onreadystatechange : 오브젝트 상태가 변한 시점에 불러오는 이벤트 핸들러 .. 즉 대기상태가 바뀌면 실행할 이벤트 
 	// callback 함수 : 서버로부터 응답이 오면 동작할 콜백함수(시스템이 자동 호출)
