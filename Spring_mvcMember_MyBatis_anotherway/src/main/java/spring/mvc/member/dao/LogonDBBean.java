...
public class LogonDBBean implements LogonDao {
	...
	public int checkPwd(Map<String, String> map) {
		...
		result = this.sqlSession.selectOne("spring.mvc.member.dao.LogonDao.check", map.get("id"));
		if (result == 1) {
			result = -1;
			int passwdCheck = this.sqlSession.selectOne("spring.mvc.member.dao.LogonDao.checkPwd", map);
			if (passwdCheck == 1) {
				result = 1;
			}
		}
	...
	public int check(String id) {
		...
		result = this.sqlSession.selectOne("spring.mvc.member.dao.LogonDao.check", id);
		return result;
	...
	public int insertMember(LogonDataBean dto) {
		...
		result = this.sqlSession.insert("spring.mvc.member.dao.LogonDao.insertMember", dto);
		return result;
		/*
		 * int result = 0; Connection con = null; PreparedStatement pstmt =
...
//		result = logonDao.deleteMember(id);
		result = this.sqlSession.delete("spring.mvc.member.dao.LogonDao.deleteMember", id);
		return result;
		/*
		 * int result = 0; Connection con = null; PreparedStatement pstmt =
		 * null; try { con = dataSource.getConnection(); String sql =
...
//		dto = logonDao.getMember(id);
		dto = this.sqlSession.selectOne("spring.mvc.member.dao.LogonDao.getMember", id);
		return dto;
		/*
		 * Connection con = null; PreparedStatement pstmt = null; ResultSet rs =
		 * null; LogonDataBean dto = new LogonDataBean(); try { con =
		 * dataSource.getConnection(); String sql =
		 * "select * from mvc_member where id=?"; pstmt =
		 * con.prepareStatement(sql); pstmt.setString(1, id); rs =
		 * pstmt.executeQuery(); if (rs.next()) { dto.setId(rs.getString("id"));
		 * dto.setPasswd(rs.getString("passwd")); dto.setName(rs.getString(3));
		 * dto.setJumin1(rs.getString(4)); dto.setJumin2(rs.getString(5));
		 * dto.setTel(rs.getString(6)); dto.setEmail(rs.getString(7));
		 * dto.setReg_date(rs.getTimestamp(8)); } } catch (SQLException e) {
		 * e.getStackTrace(); } finally { try { if (rs != null) rs.close(); if
		 * (pstmt != null) pstmt.close(); if (con != null) con.close(); } catch
		 * (SQLException e2) { e2.getStackTrace(); } } return dto;
		 */
	}
...
//		result = logonDao.modifyMember(dto);
		result = this.sqlSession.update("spring.mvc.member.dao.LogonDao.modifyMember", dto);
		return result;
		/*
		 * Connection con = null; PreparedStatement pstmt = null; int result =
		 * 0; try { con = dataSource.getConnection(); String sql =
		 * "update mvc_member set passwd=?, email=?, tel=? where id=?"; pstmt =
		 * con.prepareStatement(sql); pstmt.setString(1, dto.getPasswd());
		 * pstmt.setString(2, dto.getEmail()); pstmt.setString(3, dto.getTel());
		 * pstmt.setString(4, dto.getId()); result = pstmt.executeUpdate(); }
		 * catch (SQLException e) { e.getStackTrace(); } finally { try { if
		 * (pstmt != null) pstmt.close(); if (con != null) con.close(); } catch
		 * (Exception e2) { e2.getStackTrace(); } } return result;
		 */
